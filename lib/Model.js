...
function Model() {
	...
	if(this.container) {
		this.permissions = this.container.permissions;
	} else {
		this.permissions = {
			get:"*",
			post:"admin",
...
/**
 * Returns a given model's permissions
 */
Model.prototype.getPermissions = function(user) {
	var permissions = {};
	Object.keys(this.permissions).forEach(function(key) {
		if(user.roles.indexOf("admin")) {
			this.permissions[key].split(" ").forEach(function(role) {
